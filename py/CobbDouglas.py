...
def plotCD(fig, data, reg1, reg2, log):
	"""
	Ответственный метод построения поверхностей регрессии.
	Рекомендуется устанавливать делитель интервала с соответствием исходным данным.
	"""
	...
	for i in range (2):
		...
		ax.set_title("Постоянная отдача от масштаба" if i==0 else "Доходность с переменным масштабом", fontweight="bold")
		ax.set_xlabel('K', fontweight="bold")
		ax.set_ylabel('L', fontweight="bold")
		ax.set_zlabel('Y', fontweight="bold")
		handles, labels = ax.get_legend_handles_labels()
		ax.legend(handles, labels)
		red_patch = mpatches.Patch(color='red', label='Точки исходных данных')
...
def model():
	...
	while(True):
		try:
			file = input("Specify the name of the data file:")
			ans = input("Apply natural logarithm? (0-YES, 1-NO):")
